...
public class SimpleHeader implements Headable {
    ...
    public void setCircleColor(int color) {
    	orcPointColor = mPointColor = color;
    }
    ...
    public boolean draw(Canvas canvas, int left, int top, int right, int bottom) {
        ...
        case STATE_RELEASE:
            ...
            //add by wang
            //计算颜色渐变
            float colorR = 1f * offset / mHeight / 6;
            if (colorR < 1) {
            	float curColor = 255 * colorR;
            	int curColorInt = (int)curColor;
            	String colorStr = Integer.toHexString(curColorInt);
            	if (colorStr.length() == 1) {
            		colorStr = "0" + colorStr;
            	}
            	colorStr = colorStr + Integer.toHexString(orcPointColor).substring(2);
            	
            	mPointColor = Color.parseColor("#" + colorStr);
            } else {
            	mPointColor = orcPointColor;
            }
